...
void loop() {
  ...
  static unsigned long lastTouchTime = 0;  // Track last time any touch was processed
  static bool lastTouchProcessed = false;  // Track if we've already processed this touch down event
  
  // Per-button state tracking (to prevent toggling while held)
  static uint8_t pressedButtonRow = 255;  // Currently pressed button row (255 = none)
  static uint8_t pressedButtonCol = 255;  // Currently pressed button col (255 = none)
  static uint8_t pressedTab = 255;  // Currently pressed tab (255 = none)
  static bool orangeButtonPressed = false;  // Orange button currently pressed
  static bool orangeCirclePressed = false;  // Orange button circle currently pressed
  
  const unsigned long TOUCH_DEBOUNCE_MS = 300;  // 300ms debounce for all touches (increased from 120ms)
  ...
  // Mix of darker colors that fit the overall color scheme
  uint16_t buttonColors[2][4] = {
    {Green_Olive_Dark, Purple_Dark, Magenta_Dark, darkerGrey},  // Row 1: green, purple, magenta, dark grey
    {Pink_Magenta, Green_Olive_Dark, Purple_Dark, Orange_dark}  // Row 2: pink, green, purple, orange
  };
  ...
  // Draw bottom section UI
  layout.drawBottomSectionUI();
  
  // Draw the current page content
  layout.drawPageContent();
  ...
  { attention = digitalRead(FT5316_INT);
     ...
     // Reset button press states when touch is released
     if (touchUp) {
       lastTouchProcessed = false;
       pressedButtonRow = 255;  // Reset pressed button
       pressedButtonCol = 255;
       pressedTab = 255;  // Reset pressed tab
       orangeButtonPressed = false;  // Reset orange button pressed state
       orangeCirclePressed = false;  // Reset orange circle pressed state
     }
        ...
        for (i = 0; i < count; i++)
        {
            ...
            if ((touchLocations[i].x<50) &&(touchLocations[i].y>LCD_YSIZE_TFT-30))
              {    // Clear screen
                  ...
                  // Reset orange button state
                  layout.setOrangeButtonExtended(false);
                  ...
                  // Redraw bottom section UI
                  layout.drawBottomSectionUI();
                  
                  // Redraw the current page content
                  layout.drawPageContent();
              }  
            ...
            if (touchLocations[i].y < layout.getTopSectionY() + layout.getTopSectionHeight()) {
              ...
              if (layout.getTouchedButton(touchLocations[i].x, touchLocations[i].y, &btnRow, &btnCol)) {
                // Check if this button is already being held (prevent toggling while held)
                if (btnRow == pressedButtonRow && btnCol == pressedButtonCol) {
                  continue;  // Same button still being held, skip (no toggle)
                }
                
                // Only process if this is a new touch down event (not already processed)
                if (lastTouchProcessed) {
                ...
                lastTouchProcessed = true;  // Mark this touch down as processed
                pressedButtonRow = btnRow;  // Remember which button is now pressed
                pressedButtonCol = btnCol;
                pressedTab = 255;  // Not a tab
                orangeButtonPressed = false;  // Not orange button
                orangeCirclePressed = false;  // Not orange circle
                
                // Touch is on a button - toggle its active state (only on initial press, not while held)
                ...
                if (!currentState) {  // Now active
                  ...
                  bgColor = buttonColors[btnRow][btnCol];  // Use color from array
                  textColor = White;  // White text for dark backgrounds
                }
              ...
              // Then check if touch is on a tab
                ...
                if (touchedTab < 255) {
                  // Check if this tab is already being held (prevent retriggering while held)
                  if (touchedTab == pressedTab) {
                    continue;  // Same tab still being held, skip
                  }
                  ...
                  lastTouchTime = currentTime;  // Update last touch time
                  lastTouchProcessed = true;  // Mark this touch down as processed
                  pressedTab = touchedTab;  // Remember which tab is now pressed
                  pressedButtonRow = 255;  // Not a button
                  pressedButtonCol = 255;
                  orangeButtonPressed = false;  // Not orange button
                  orangeCirclePressed = false;  // Not orange circle
                  ...
                  if (previousActiveTab < 255 && previousActiveTab != touchedTab) {
                    ...
                    layout.drawTabLabel(previousActiveTab);
                    layout.drawTabValue(previousActiveTab);
                  }
                  ...
                  layout.drawTabLabel(touchedTab);
                  layout.drawTabValue(touchedTab);
                }
              }
            }
            else {
              // Touch is not in top section - check bottom section
              // Apply same debouncing as all other buttons
              if (lastTouchProcessed) {
                continue;  // Already processed this touch down, skip
              }
              ...
              // When orange button is extended, check circles (including "pge")
              if (layout.isOrangeButtonExtended()) {
                int8_t touchedCircle = layout.getTouchedExtendedCircle(touchLocations[i].x, touchLocations[i].y);
                if (touchedCircle == -2) {
                  // "pge" button was touched
                  // Check if already being held (prevent retriggering while held)
                  if (orangeCirclePressed) {
                    continue;  // Still being held, skip
                  }
                  
                  // "pge" button was touched - navigate to central page
                  lastTouchTime = currentTime;  // Update last touch time
                  lastTouchProcessed = true;  // Mark this touch down as processed
                  orangeCirclePressed = true;  // Remember circle is pressed
                  pressedButtonRow = 255;  // Not a button
                  pressedButtonCol = 255;
                  pressedTab = 255;  // Not a tab
                  orangeButtonPressed = false;  // Not the button itself
                  
                  layout.setCurrentPage(PAGE_CENTRAL);
                  layout.setOrangeButtonExtended(false);  // Collapse button after navigation
                  
                  // Redraw everything for the new page
                  layout.drawBottomSectionUI();
                  layout.drawPageContent();
                  continue;  // Skip debug drawing
                } else if (touchedCircle >= 0) {
                  // Check if orange circle is already being held (prevent retriggering while held)
                  if (orangeCirclePressed) {
                    continue;  // Orange circle still being held, skip
                  }
                  
                  // Circle was touched - navigate to corresponding page
                  lastTouchTime = currentTime;  // Update last touch time
                  lastTouchProcessed = true;  // Mark this touch down as processed
                  orangeCirclePressed = true;  // Remember orange circle is pressed
                  pressedButtonRow = 255;  // Not a button
                  pressedButtonCol = 255;
                  pressedTab = 255;  // Not a tab
                  orangeButtonPressed = false;  // Not the button itself
                  
                  // Navigate to page based on circle: 0=ext, 1=mid, 2=set
                  PageType targetPage;
                  if (touchedCircle == 0) {
                    targetPage = PAGE_EXT;
                  } else if (touchedCircle == 1) {
                    targetPage = PAGE_MIDI;
                  } else {  // touchedCircle == 2
                    targetPage = PAGE_SETTINGS;
                  }
                  
                  layout.setCurrentPage(targetPage);
                  layout.setOrangeButtonExtended(false);  // Collapse button after navigation
              ...
              // Check if touch is on central page buttons
              int8_t touchedPageButton = layout.getTouchedCentralPageButton(touchLocations[i].x, touchLocations[i].y);
              if (touchedPageButton >= 0) {
                // Check if already being held (prevent retriggering while held)
                if (lastTouchProcessed) {
                  continue;  // Already processed this touch down, skip
                }
                ...
                PageType targetPage = (PageType)touchedPageButton;
                layout.setCurrentPage(targetPage);
                
                // Redraw page content and bottom section UI (to update orange button text)
                layout.drawPageContent();
                layout.drawBottomSectionUI();
                continue;  // Skip debug drawing
              } else if (touchedPageButton == -2) {
                // "back" button was touched - go back to previous page (or global)
                if (lastTouchProcessed) {
                  continue;  // Already processed this touch down, skip
                }
                ...
                // Go back to global page
                layout.setCurrentPage(PAGE_GLOBAL);
              ...
              // Check if touch is on orange button (contracted area only, even when extended)
              // The button clickable area stays the same size regardless of extension
              if (layout.isTouchedOrangeButton(touchLocations[i].x, touchLocations[i].y)) {
                // Check if orange button is already being held (prevent retriggering while held)
                if (orangeButtonPressed) {
                  continue;  // Orange button still being held, skip
                }
                
                if (!layout.isOrangeButtonExtended()) {
                  // Button is not extended - extend it
                  lastTouchTime = currentTime;  // Update last touch time
                  lastTouchProcessed = true;  // Mark this touch down as processed
                  orangeButtonPressed = true;  // Remember orange button is pressed
                  pressedButtonRow = 255;  // Not a button
                  pressedButtonCol = 255;
                  pressedTab = 255;  // Not a tab
                  orangeCirclePressed = false;  // Not a circle
                  layout.setOrangeButtonExtended(true);
                  // Redraw bottom section
                  layout.drawBottomSectionUI();
                  continue;  // Skip debug drawing
                } else {
                  // Button is extended - clicking contracted area collapses it
                  lastTouchTime = currentTime;  // Update last touch time
                  lastTouchProcessed = true;  // Mark this touch down as processed
                  orangeButtonPressed = true;  // Remember orange button is pressed
                  pressedButtonRow = 255;  // Not a button
